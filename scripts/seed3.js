...
async function seed() {
  ...
  for (let classroom of classroomSeed) {
    ...
    const currentClassroom = await Classroom.findById(currentClassroomId);
    console.log("currentClassroom", currentClassroom);

    try {
      let insertedAssignmentsId = [];
      const assignmentsArray = classroom.assignments.map(assignment => ({
        ...assignment,
        classroomId: currentClassroomId
      }));
      await Assignment.insertMany(assignmentsArray, function(error, docs) {
        for (let i = 0; i < docs.length; i++) {
          const currentAssignmentId = docs[i]._id;
          insertedAssignmentsId.push(currentAssignmentId);
          currentClassroom.assignmentsId.push(currentAssignmentId);
          Classroom.findByIdAndUpdate(
            currentClassroomId,
            { assignmentsId: currentClassroom.assignmentsId },
            function(err, callback) {
              console.log("Add assignmentsId to classroom", callback);
            }
          );
        }
      });
